...
object PedidoRequests {
    ...
    fun GetAllByGinasioID(scope: CoroutineScope, token : String?, targetID : Int?, callback: (ArrayList<Pedido>) -> Unit){
        scope.launch(Dispatchers.IO) {
            val request = Request.Builder()
                .url(UtilsForRequests.baseURL + "/api/Pedido/Pedidos/Ginasio/$targetID")
                .get()
                .header("Authorization", "Bearer $token")
                .build()
