...
import vough.example.ipcagym.cliente_classes.ActivityDetailClienteActivity
...
class FuncionarioManagementActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        receiverNewActivity = registerForActivityResult(ActivityResultContracts.StartActivityForResult()){
                    ...
                    for(activity in activityList){
                        if(id_ginasio == activity.id_ginasio && id_cliente == activity.id_cliente
                            ...
                            if(activity.data_saida == null){
                                Toast.makeText(this@FuncionarioManagementActivity,"This client needs to exit first!", Toast.LENGTH_LONG).show()
                            }
                            else
                            {
                                activityList.add(Atividade(id_atividade, id_ginasio, id_cliente, LocalDateTime.now(), null))
                                Toast.makeText(this@FuncionarioManagementActivity,"Activity added successfully", Toast.LENGTH_LONG).show()
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    for(activity in activityList){
                        if(id_ginasio == activity.id_ginasio && id_cliente == activity.id_cliente
                            ...
                            {
                                Toast.makeText(this@FuncionarioManagementActivity,"This client needs to enter first!", Toast.LENGTH_LONG).show()
                            }
                        }
                    }
            ...
            {
                Toast.makeText(this@FuncionarioManagementActivity,"Error on adding a new activity", Toast.LENGTH_LONG).show()
            }
        }

        addButton.setOnClickListener {
            receiverNewActivity?.launch(Intent(this@FuncionarioManagementActivity, ActivityAddActivity::class.java))
        }
    }
    ...
    inner class FuncionarioActivityAdapter : BaseAdapter(){
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup?): View {
            ...
            rootView.setOnClickListener {
                val intent = Intent(this@FuncionarioManagementActivity, FuncionarioManagementDetailsActivity::class.java)
