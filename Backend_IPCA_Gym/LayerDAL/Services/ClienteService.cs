...
namespace LayerDAL.Services
{
    public class ClienteService
    {
        ...
        public static async Task<bool> PatchService(string sqlDataSource, Cliente cliente, int targetID)
        {
            ...
            try
            {
                Cliente clienteAnterior = await GetByIDService(sqlDataSource, targetID);
                SqlDataReader dataReader;
                ...
                using (SqlConnection databaseConnection = new SqlConnection(sqlDataSource))
                {
                    ...
                    using (SqlCommand myCommand = new SqlCommand(query, databaseConnection))
                    {
                        myCommand.Parameters.AddWithValue("id_cliente", cliente.id_cliente != 0 ? cliente.id_cliente : clienteAnterior.id_cliente);
                        myCommand.Parameters.AddWithValue("id_ginasio", cliente.id_ginasio != 0 ? cliente.id_ginasio : clienteAnterior.id_ginasio);
                        
                        Console.WriteLine($"Boas -> {cliente.id_plano_nutricional}");
                        
                        if(cliente.id_plano_nutricional != null)
                            myCommand.Parameters.AddWithValue("id_plano_nutricional", cliente.id_plano_nutricional != null ? cliente.id_plano_nutricional : clienteAnterior.id_plano_nutricional);
                        else
                            myCommand.Parameters.AddWithValue("id_plano_nutricional", DBNull.Value);

                        myCommand.Parameters.AddWithValue("nome", !string.IsNullOrEmpty(cliente.nome) ? cliente.nome : clienteAnterior.nome);
                        myCommand.Parameters.AddWithValue("mail", !string.IsNullOrEmpty(cliente.mail) ? cliente.mail : clienteAnterior.mail);
                        myCommand.Parameters.AddWithValue("telemovel", cliente.telemovel != 0 ? cliente.telemovel : clienteAnterior.telemovel);
                        myCommand.Parameters.AddWithValue("pass_hash", clienteAnterior.pass_hash);
                        myCommand.Parameters.AddWithValue("pass_salt", clienteAnterior.pass_salt);

                        if (cliente.peso != null)
                            myCommand.Parameters.AddWithValue("peso", cliente.peso);
                        else
                            myCommand.Parameters.AddWithValue("peso", clienteAnterior.peso != null ? clienteAnterior.peso : DBNull.Value);

                        if (cliente.altura != null)
                            myCommand.Parameters.AddWithValue("altura", cliente.altura);
                        else
                            myCommand.Parameters.AddWithValue("altura", clienteAnterior.altura != null ? clienteAnterior.altura : DBNull.Value);

                        if (cliente.gordura != null)
                            myCommand.Parameters.AddWithValue("gordura", cliente.gordura);
                        else
                            myCommand.Parameters.AddWithValue("gordura", clienteAnterior.gordura != null ? clienteAnterior.gordura : DBNull.Value);

                        if (cliente.foto_perfil != null)
                            myCommand.Parameters.AddWithValue("foto_perfil", cliente.foto_perfil);
                        else
                            myCommand.Parameters.AddWithValue("foto_perfil", clienteAnterior.foto_perfil != null ? clienteAnterior.foto_perfil : DBNull.Value);

                        myCommand.Parameters.AddWithValue("estado", !string.IsNullOrEmpty(cliente.estado) ? cliente.estado : clienteAnterior.estado);
        ...
        /// <exception cref="ArgumentNullException">Ocorre quando um parâmetro é nulo.</exception>
        /// <exception cref="Exception">Ocorre quando ocorre qualquer outro erro.</exception>
        public static async Task<LoginModel> LoginService(string sqlDataSource, LoginCliente conta, IConfiguration _configuration)
        {
            string query = @"
                ...
                using (SqlConnection databaseConnection = new SqlConnection(sqlDataSource))
                {
                    ...
                    using (SqlCommand myCommand = new SqlCommand(query, databaseConnection))
                    {
                        ...
                        using (SqlDataReader reader = myCommand.ExecuteReader())
                        {
                            ...
                            LoginModel loginModel = Token.CreateTokenCliente(targetCliente, _configuration);
        ...
        /// <summary>
        /// Remoção de um utilizador através do seu id
        /// </summary>
        /// <param name="sqlDataSource">String de conexão com a base de dados</param>
        ...
        /// <exception cref="ArgumentNullException">Ocorre quando um parâmetro é nulo.</exception>
        /// <exception cref="Exception">Ocorre quando ocorre qualquer outro erro.</exception>
        public static async Task<bool> DeleteClienteService(string sqlDataSource, int targetID)
        {
            try
            {
                // Inativar marcações
                List<Marcacao> marcacoesCliente = await MarcacaoService.GetAllByClienteIDService(sqlDataSource, targetID);
                foreach (Marcacao marcacao in marcacoesCliente)
                {
                ...
                // Inativar os pedidos
                List<Pedido> pedidosCliente = await PedidoService.GetAllByClienteIDService(sqlDataSource, targetID);
                foreach (Pedido pedidoCliente in pedidosCliente)
                {
                    PedidoLojaService.DeletePedidoService(sqlDataSource, pedidoCliente.id_pedido);
                }
